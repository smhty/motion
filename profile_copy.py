...
pa: portion of td that the robot is accelerating and decelerating
...
vm = (d - v0 * td*pa/2) / (td*(1-pa/2))
"""
def profile_1_time(j, d, t_d, p_a=0.5, v_0 = 0):
	m = p_a*t_d

	# t_4
	t_4 = math.floor(t_d - m)
	v_m = (d - v_0 * m /2)/(t_4 + m / 2)

	if (v_m - v_0)/j >= m**2 / 16: # increase j
		t_1 = math.floor(m/8) # t_1 >=1
		t_2 = math.floor(m/2 - 2*t_1)
	else:
		print("ali")
		t_1 = m/4 - (1/2)* math.sqrt(m**2 / 4 - 4*(v_m-v_0)/j) # pick the smaller solution
		t_1 = math.floor(t_1)
		t_2 = math.floor(m/2 - 2*t_1)
...
class type_2(object):
	...
	p_a = (2*t_1 + t_2) / t_d
	"""
	def time(self, j, d, t_d, p_a=0.5, v_0 = 0):
		m = p_a * t_d
...
def profile_2_time(j, d, t_d, p_a=0.5, v_0 = 0):
	return 0
"""
...
def main_1():
	...
	result = profile_1_time(j, d, t_d, p_a, v_0)
